Feature: Match Recording

  Background:
    Given I am logged in as admin
    And I am on the admin page

  Scenario Outline: Successfully record a match result with a final score
...
Scenario: Record a match with goal scorers
  ...
  And  each player's total goal count should increase accordingly
...
Scenario: Record a match with a player scoring an own goal
  ...
  Then the match should appear in the match history with corresponding scorers
  And  each player's total goal count should increase accordingly
  # Own-goal goals don't count toward the scorer's personal total in the Top Scorers list
