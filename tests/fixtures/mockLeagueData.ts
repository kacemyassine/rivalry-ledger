import { Match } from "@/store/leagueStore";

// ================================================================================
// Base data — no matches, all stats zero
// ================================================================================

export const mockLeagueData = {
  leagueConfig: { id: "test-league", name: "Test League" },
  targetMatches: 10,
  teams: [
    {
      id: "team-1",
      name: "Harbor United",
      coach: "Coach A",
      logo: "",
      played: 0,
      won: 0,
      drawn: 0,
      lost: 0,
      goalsFor: 0,
      goalsAgainst: 0,
      points: 0,
    },
    {
      id: "team-2",
      name: "Ocean Dragon",
      coach: "Coach B",
      logo: "",
      played: 0,
      won: 0,
      drawn: 0,
      lost: 0,
      goalsFor: 0,
      goalsAgainst: 0,
      points: 0,
    },
  ],
  players: [
    {
      id: "player-1",
      name: "Antoine Griezmann",
      teamId: "team-1",
      goals: 0,
      image: "",
      fullImage: "",
    },
    {
      id: "player-2",
      name: "Kylian Mbappé",
      teamId: "team-1",
      goals: 0,
      image: "",
      fullImage: "",
    },
    {
      id: "player-3",
      name: "Lionel Messi",
      teamId: "team-1",
      goals: 0,
      image: "",
      fullImage: "",
    },
    {
      id: "player-4",
      name: "Frank Lampard",
      teamId: "team-1",
      goals: 0,
      image: "",
      fullImage: "",
    },
    {
      id: "player-5",
      name: "Johan Cruyff",
      teamId: "team-1",
      goals: 0,
      image: "",
      fullImage: "",
    },
    {
      id: "player-6",
      name: "Ruud Gullit",
      teamId: "team-2",
      goals: 0,
      image: "",
      fullImage: "",
    },
    {
      id: "player-7",
      name: "Didier Drogba",
      teamId: "team-2",
      goals: 0,
      image: "",
      fullImage: "",
    },
    {
      id: "player-8",
      name: "Petr Čech",
      teamId: "team-2",
      goals: 0,
      image: "",
      fullImage: "",
    },
    {
      id: "player-9",
      name: "Désiré Doué",
      teamId: "team-2",
      goals: 0,
      image: "",
      fullImage: "",
    },
    {
      id: "player-10",
      name: "Cristiano Ronaldo",
      teamId: "team-2",
      goals: 0,
      image: "",
      fullImage: "",
    },
  ],
  matches: [],
};

// ================================================================================
// mockMatchesWithoutScorers — 11 matches, no scorer data
// ================================================================================

export const mockMatchesWithoutScorers: Match[] = [
  {
    id: "match-1",
    homeTeamId: "team-1",
    awayTeamId: "team-2",
    homeTeamName: "Harbor United",
    awayTeamName: "Ocean Dragon",
    homeGoals: 2,
    awayGoals: 1,
    scorers: [],
    date: "2026-01-01",
  },
  {
    id: "match-2",
    homeTeamId: "team-2",
    awayTeamId: "team-1",
    homeTeamName: "Ocean Dragon",
    awayTeamName: "Harbor United",
    homeGoals: 0,
    awayGoals: 0,
    scorers: [],
    date: "2026-01-08",
  },
  {
    id: "match-3",
    homeTeamId: "team-1",
    awayTeamId: "team-2",
    homeTeamName: "Harbor United",
    awayTeamName: "Ocean Dragon",
    homeGoals: 3,
    awayGoals: 2,
    scorers: [],
    date: "2026-01-15",
  },
  {
    id: "match-4",
    homeTeamId: "team-2",
    awayTeamId: "team-1",
    homeTeamName: "Ocean Dragon",
    awayTeamName: "Harbor United",
    homeGoals: 1,
    awayGoals: 1,
    scorers: [],
    date: "2026-01-22",
  },
  {
    id: "match-5",
    homeTeamId: "team-1",
    awayTeamId: "team-2",
    homeTeamName: "Harbor United",
    awayTeamName: "Ocean Dragon",
    homeGoals: 4,
    awayGoals: 0,
    scorers: [],
    date: "2026-01-29",
  },
  {
    id: "match-6",
    homeTeamId: "team-2",
    awayTeamId: "team-1",
    homeTeamName: "Ocean Dragon",
    awayTeamName: "Harbor United",
    homeGoals: 2,
    awayGoals: 3,
    scorers: [],
    date: "2026-02-05",
  },
  {
    id: "match-7",
    homeTeamId: "team-1",
    awayTeamId: "team-2",
    homeTeamName: "Harbor United",
    awayTeamName: "Ocean Dragon",
    homeGoals: 1,
    awayGoals: 1,
    scorers: [],
    date: "2026-02-12",
  },
  {
    id: "match-8",
    homeTeamId: "team-2",
    awayTeamId: "team-1",
    homeTeamName: "Ocean Dragon",
    awayTeamName: "Harbor United",
    homeGoals: 0,
    awayGoals: 2,
    scorers: [],
    date: "2026-02-19",
  },
  {
    id: "match-9",
    homeTeamId: "team-1",
    awayTeamId: "team-2",
    homeTeamName: "Harbor United",
    awayTeamName: "Ocean Dragon",
    homeGoals: 1,
    awayGoals: 3,
    scorers: [],
    date: "2026-02-26",
  },
  {
    id: "match-10",
    homeTeamId: "team-2",
    awayTeamId: "team-1",
    homeTeamName: "Ocean Dragon",
    awayTeamName: "Harbor United",
    homeGoals: 2,
    awayGoals: 2,
    scorers: [],
    date: "2026-03-05",
  },
  {
    id: "match-11",
    homeTeamId: "team-1",
    awayTeamId: "team-2",
    homeTeamName: "Harbor United",
    awayTeamName: "Ocean Dragon",
    homeGoals: 5,
    awayGoals: 1,
    scorers: [],
    date: "2026-03-12",
  },
];
// ================================================================================
// With matches — team stats reflect mockMatchesWithoutScorers, no scorer data
// ================================================================================

export const mockLeagueDataWithMatches = {
  ...mockLeagueData,
  teams: [
    {
      id: "team-1",
      name: "Harbor United",
      coach: "Coach A",
      logo: "",
      played: 11,
      won: 6,
      drawn: 4,
      lost: 1,
      goalsFor: 24,
      goalsAgainst: 13,
      points: 0,
    },
    {
      id: "team-2",
      name: "Ocean Dragon",
      coach: "Coach B",
      logo: "",
      played: 11,
      won: 1,
      drawn: 4,
      lost: 6,
      goalsFor: 13,
      goalsAgainst: 24,
      points: 0,
    },
  ],
  matches: mockMatchesWithoutScorers,
};

// ================================================================================
// mockMatchesWithScorers — same 11 matches, with scorer data
// ================================================================================

export const mockMatchesWithScorers: Match[] = [
  {
    id: "match-1",
    homeTeamId: "team-1",
    awayTeamId: "team-2",
    homeTeamName: "Harbor United",
    awayTeamName: "Ocean Dragon",
    homeGoals: 3,
    awayGoals: 2,
    scorers: [
      { playerId: "player-1", goals: 2, isOwnGoal: false },
      { playerId: "player-2", goals: 1, isOwnGoal: false },
      { playerId: "player-6", goals: 1, isOwnGoal: false },
      { playerId: "player-7", goals: 1, isOwnGoal: false },
    ],
    date: "2026-01-01",
  },
  {
    id: "match-2",
    homeTeamId: "team-2",
    awayTeamId: "team-1",
    homeTeamName: "Ocean Dragon",
    awayTeamName: "Harbor United",
    homeGoals: 2,
    awayGoals: 1,
    scorers: [
      { playerId: "player-10", goals: 2, isOwnGoal: false },
      { playerId: "player-3", goals: 1, isOwnGoal: false },
    ],
    date: "2026-01-08",
  },
  {
    id: "match-3",
    homeTeamId: "team-1",
    awayTeamId: "team-2",
    homeTeamName: "Harbor United",
    awayTeamName: "Ocean Dragon",
    homeGoals: 2,
    awayGoals: 1,
    scorers: [
      { playerId: "player-3", goals: 1, isOwnGoal: false },
      { playerId: "player-5", goals: 1, isOwnGoal: false },
      { playerId: "player-9", goals: 1, isOwnGoal: false },
    ],
    date: "2026-01-15",
  },
  {
    id: "match-4",
    homeTeamId: "team-2",
    awayTeamId: "team-1",
    homeTeamName: "Ocean Dragon",
    awayTeamName: "Harbor United",
    homeGoals: 3,
    awayGoals: 1,
    scorers: [
      { playerId: "player-6", goals: 2, isOwnGoal: false },
      { playerId: "player-10", goals: 1, isOwnGoal: false },
      { playerId: "player-4", goals: 1, isOwnGoal: false },
    ],
    date: "2026-01-22",
  },
  {
    id: "match-5",
    homeTeamId: "team-1",
    awayTeamId: "team-2",
    homeTeamName: "Harbor United",
    awayTeamName: "Ocean Dragon",
    homeGoals: 1,
    awayGoals: 1,
    scorers: [
      { playerId: "player-1", goals: 1, isOwnGoal: false },
      { playerId: "player-7", goals: 1, isOwnGoal: false },
    ],
    date: "2026-01-29",
  },
  {
    id: "match-6",
    homeTeamId: "team-2",
    awayTeamId: "team-1",
    homeTeamName: "Ocean Dragon",
    awayTeamName: "Harbor United",
    homeGoals: 2,
    awayGoals: 2,
    scorers: [
      { playerId: "player-8", goals: 1, isOwnGoal: false },
      { playerId: "player-9", goals: 1, isOwnGoal: false },
      { playerId: "player-2", goals: 1, isOwnGoal: false },
      { playerId: "player-4", goals: 1, isOwnGoal: false },
    ],
    date: "2026-02-05",
  },
  {
    id: "match-7",
    homeTeamId: "team-1",
    awayTeamId: "team-2",
    homeTeamName: "Harbor United",
    awayTeamName: "Ocean Dragon",
    homeGoals: 4,
    awayGoals: 1,
    scorers: [
      { playerId: "player-1", goals: 2, isOwnGoal: false },
      { playerId: "player-3", goals: 1, isOwnGoal: false },
      { playerId: "player-5", goals: 1, isOwnGoal: false },
      { playerId: "player-10", goals: 1, isOwnGoal: false },
    ],
    date: "2026-02-12",
  },
  {
    id: "match-8",
    homeTeamId: "team-2",
    awayTeamId: "team-1",
    homeTeamName: "Ocean Dragon",
    awayTeamName: "Harbor United",
    homeGoals: 1,
    awayGoals: 3,
    scorers: [
      { playerId: "player-7", goals: 1, isOwnGoal: false },
      { playerId: "player-2", goals: 2, isOwnGoal: false },
      { playerId: "player-3", goals: 1, isOwnGoal: false },
    ],
    date: "2026-02-19",
  },
  {
    id: "match-9",
    homeTeamId: "team-1",
    awayTeamId: "team-2",
    homeTeamName: "Harbor United",
    awayTeamName: "Ocean Dragon",
    homeGoals: 2,
    awayGoals: 3,
    scorers: [
      { playerId: "player-1", goals: 1, isOwnGoal: false },
      { playerId: "player-5", goals: 1, isOwnGoal: false },
      { playerId: "player-6", goals: 2, isOwnGoal: false },
      { playerId: "player-8", goals: 1, isOwnGoal: false },
    ],
    date: "2026-02-26",
  },
  {
    id: "match-10",
    homeTeamId: "team-2",
    awayTeamId: "team-1",
    homeTeamName: "Ocean Dragon",
    awayTeamName: "Harbor United",
    homeGoals: 0,
    awayGoals: 2,
    scorers: [
      { playerId: "player-1", goals: 1, isOwnGoal: false },
      { playerId: "player-3", goals: 1, isOwnGoal: false },
    ],
    date: "2026-03-05",
  },
  {
    id: "match-11",
    homeTeamId: "team-1",
    awayTeamId: "team-2",
    homeTeamName: "Harbor United",
    awayTeamName: "Ocean Dragon",
    homeGoals: 3,
    awayGoals: 2,
    scorers: [
      { playerId: "player-2", goals: 1, isOwnGoal: false },
      { playerId: "player-4", goals: 1, isOwnGoal: false },
      { playerId: "player-5", goals: 1, isOwnGoal: false },
      { playerId: "player-9", goals: 1, isOwnGoal: false },
      { playerId: "player-10", goals: 1, isOwnGoal: false },
    ],
    date: "2026-03-12",
  },
];

// ================================================================================
// With scorers — team stats + player goals reflect mockMatchesWithScorers
// ================================================================================

export const mockLeagueDataWithScorers = {
  ...mockLeagueData,
  teams: [
    {
      id: "team-1",
      name: "Harbor United",
      coach: "Coach A",
      logo: "",
      played: 11,
      won: 6,
      drawn: 2,
      lost: 3,
      goalsFor: 24,
      goalsAgainst: 18,
      points: 0,
    },
    {
      id: "team-2",
      name: "Ocean Dragon",
      coach: "Coach B",
      logo: "",
      played: 11,
      won: 3,
      drawn: 2,
      lost: 6,
      goalsFor: 18,
      goalsAgainst: 24,
      points: 0,
    },
  ],
  players: [
    {
      id: "player-1",
      name: "Antoine Griezmann",
      teamId: "team-1",
      goals: 7,
      image: "",
      fullImage: "",
    },
    {
      id: "player-2",
      name: "Kylian Mbappé",
      teamId: "team-1",
      goals: 5,
      image: "",
      fullImage: "",
    },
    {
      id: "player-3",
      name: "Lionel Messi",
      teamId: "team-1",
      goals: 5,
      image: "",
      fullImage: "",
    },
    {
      id: "player-4",
      name: "Frank Lampard",
      teamId: "team-1",
      goals: 3,
      image: "",
      fullImage: "",
    },
    {
      id: "player-5",
      name: "Johan Cruyff",
      teamId: "team-1",
      goals: 4,
      image: "",
      fullImage: "",
    },
    {
      id: "player-6",
      name: "Ruud Gullit",
      teamId: "team-2",
      goals: 5,
      image: "",
      fullImage: "",
    },
    {
      id: "player-7",
      name: "Didier Drogba",
      teamId: "team-2",
      goals: 3,
      image: "",
      fullImage: "",
    },
    {
      id: "player-8",
      name: "Petr Čech",
      teamId: "team-2",
      goals: 2,
      image: "",
      fullImage: "",
    },
    {
      id: "player-9",
      name: "Désiré Doué",
      teamId: "team-2",
      goals: 3,
      image: "",
      fullImage: "",
    },
    {
      id: "player-10",
      name: "Cristiano Ronaldo",
      teamId: "team-2",
      goals: 5,
      image: "",
      fullImage: "",
    },
  ],
  matches: mockMatchesWithScorers,
};
