import { MATCH_ERRORS, PLAYER_ERRORS, TEAM_ERRORS } from "./errorMessages";
import { mockLeagueData } from "./mockLeagueData";

export function runMatchValidationTests(action: (homeGoals: number, awayGoals: number, scorers: { playerId: string; goals: number; isOwnGoal?: boolean }[]) => void) {
  ...
  test.skip("throws an error if goals are unrealistically high", () => {
    expect(() => action(999, 999, [])).toThrow(MATCH_ERRORS.GOALS_UNREALISTIC);
  });
  ...
  test.skip("throws an error if scorer goals sum exceeds total goals", () => {
    expect(() =>
      action(2, 1, [
        ...
        { playerId: mockLeagueData.players[1].id, goals: 2 },
      ]),
    ).toThrow(MATCH_ERRORS.SCORER_GOALS_MISMATCH);
  });
  ...
  test.skip("throws an error if scorers data contains a playerId that belongs to the opposing team", () => {
    expect(() =>
      action(2, 1, [{ playerId: mockLeagueData.players[6].id, goals: 1 }]),
