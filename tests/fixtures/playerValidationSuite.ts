...
export function runNameValidationTests(action: (name: string) => void) {
  ...
  test.skip("throws an error if player name below minimum length", () => {
  ...
  test.skip("trims leading and trailing whitespace before validation", () => {
  ...
  test.skip("player name with exactly minimum length characters is allowed", () => {
    expect(() => action("a".repeat(PLAYER_NAME_RULES.minLength))).not.toThrow();
  });

  // BVA: maximum valid boundary — exactly maxLength characters
  test.skip("player name with exactly maximum length characters is allowed", () => {
  ...
  test.skip("throws an error if player name exceeds maximum length", () => {
