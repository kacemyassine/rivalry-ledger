...
export function MatchForm({
  ...
  const handleAddScorer = () => {
    ...
    setScorers((s) => [
      ...s,
      { playerId: players[0].id, goals: 1, isOwnGoal: false },
    ]);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (leagueType === "with-scorers" || scorers.length > 0) {
      ...
      const effectiveAwayGoals = calculateEffectiveGoals(
        scorers,
        players,
        selectedAwayTeam.id,
      );
    ...
    if (editingMatch) {
      ...
      try {
        useLeagueStore
          .getState()
          .editMatch(editingMatch.id, homeGoals, awayGoals, scorers, newDate);
      ...
      try {
        ...
        toast.error(e instanceof Error ? e.message : "Failed to record match");
        return;
      }
    }
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-6 pt-4">
          ...
          >
            ...
            <span className="text-xl md:text-2xl text-muted-foreground font-display pb-3 md:pb-4">
              VS
            </span>
            <div className="text-center flex-1">
              <p className="text-xs md:text-sm text-muted-foreground mb-2">
                {selectedAwayTeam?.name || "Away Team"}
              </p>
              <Input
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <Button
                type="button"
                data-testid="add-scorer-btn"
                variant="outline"
                size="sm"
                onClick={handleAddScorer}
                className="h-8 border-yellow-400/30 text-yellow-300 hover:bg-yellow-400/10 hover:text-yellow-200 bg-transparent"
              >
              ...
              >
                ...
                <div className="flex items-center gap-1 shrink-0">
                  <input
                    ...
                    onChange={(e) =>
                      handleScorerChange(index, "isOwnGoal", e.target.checked)
                    }
                    className="w-4 h-4 accent-red-500 cursor-pointer"
                  />
                  <label
                    htmlFor={`og-${index}`}
                    className="text-xs text-red-400 cursor-pointer whitespace-nowrap"
                  >
                    OG
                  </label>
                </div>
