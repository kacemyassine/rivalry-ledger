  ...
  return (
    <>
      ...
      <div className="hidden md:flex fixed top-6 right-6 z-50 flex-col items-end gap-2 max-w-sm">
        <div className="bg-yellow-400 rounded-2xl shadow-[0_0_30px_rgba(234,179,8,0.4)] overflow-hidden w-full">
          {/* Header */}
          <div className="flex items-center gap-2 px-4 py-3 border-b border-[#0a0e2a]/20">
            <AlertTriangle className="w-4 h-4 text-[#0a0e2a] shrink-0" />
            <span className="text-[#0a0e2a] font-semibold text-sm flex-1">
            ...
            <Button
              ...
              disabled={saving}
              size="sm"
              className="bg-[#0a0e2a] hover:bg-[#0d1133] text-yellow-400 font-bold h-7 px-3 text-xs"
            >
          ...
          <div className="max-h-48 overflow-y-auto px-4 py-2 space-y-1">
            {changeLog.map((entry, i) => (
              <p key={i} className="text-xs text-[#0a0e2a]/70 py-1 border-b border-[#0a0e2a]/10 last:border-0">
                • {entry}
              </p>
            ))}
          </div>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50">
        <div className="bg-yellow-400 shadow-[0_-4px_20px_rgba(234,179,8,0.4)]">
          {/* Expanded change log */}
          {expanded && (
          ...
          <div className="flex items-center gap-3 px-4 py-3">
            ...
            >
              <span className="text-[#0a0e2a] font-semibold text-sm">
                {changeLog.length} Unsaved Change{changeLog.length > 1 ? 's' : ''}
              </span>
              {expanded
                ? <ChevronDown className="w-4 h-4 text-[#0a0e2a]/60" />
                : <ChevronUp className="w-4 h-4 text-[#0a0e2a]/60" />
              }
            </button>
            <Button
              onClick={onSave}
